...
class SemanticAnalyzer:
    def __init__(self):
        # solo un scope global para este mini-lenguaje
    ...
    def check_stmt(self, stmt: Stmt):
        if isinstance(stmt, VarDecl):
            ...
            self.env[stmt.name] = stmt.var_type
            ...
            if t_expr != t_var:
                raise SemanticError(f"Type mismatch in assignment to '{stmt.name}': {t_var} = {t_expr}")
            ...
            _ = self.check_expr(stmt.expr)  # allow int or bool

        elif isinstance(stmt, Block):
            # para este proyecto, usamos un solo scope global,
            # pero aun así recorremos las statements
